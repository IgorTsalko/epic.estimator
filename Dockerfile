...
COPY --chown=101:101 config/docker-entrypoint.d/20-render-config.sh /docker-entrypoint.d/20-render-config.sh
...
RUN mkdir -p /var/log/nginx && \
    ...
    chmod +x /docker-entrypoint.d/20-render-config.sh
